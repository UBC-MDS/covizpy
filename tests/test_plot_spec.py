...
def test_plot_spec_inputs(df):
    ...
    assert "Invalid argument type: values inside location list must be strings." == str(
        e.value
    )
...
def test_plot_spec_mapping(df):
    ...
    assert (
        plot_spec(df, date_from="2022-01-19", date_to="2022-01-20")
        .layer[0]
        .data["new_cases"]
        == [16849, 15775]
    ).all(), "Altair chart data is wrong"
