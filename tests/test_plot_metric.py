...
def test_plot_metric_input():
    ...
    assert 'Incorrect argument type: The starting date should be in string format' == str(
        e.value)
    ...
    assert 'Incorrect argument type: Metric 1 input should be a string' == str(
        e.value)

     # check if the metric provided is one of the columns in the dataframe
    with raises(ValueError) as e:
        plot_metric(metric="invalid_column")
    assert 'Incorrect argument value: The metric chosen is not one of the columns in dataframe' == str(
        e.value)
...
def test_plot_metric_output():
    ...
    assert plot_metric().layer[0].mark.type == 'line', "Incorrect Mark Type: Layer 1 mark is not line"

    # check the layer 2 type is line or not
    assert plot_metric().layer[1].mark.type == 'line', "Incorrect Mark Type: Layer 2 mark is not line"

    # check the layer 1 x-axis is named Date or not
    assert plot_metric().layer[0].encoding.x.title == 'Date', "Incorrect Axis Label: Layer 1 x-axis label is not Date"
